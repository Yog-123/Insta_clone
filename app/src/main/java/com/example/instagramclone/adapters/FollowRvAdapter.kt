...
class FollowRvAdapter (var context: Context, var followList: ArrayList<User>) :
    ...
    inner class ViewHolder(var binding: FollowRvBinding) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        Glide.with(context).load(followList.get(position).image).placeholder(R.drawable.user).into(holder.binding.profileMm)
        holder.binding.nameFll.text=followList.get(position).name
